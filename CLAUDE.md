...
- npm auth is **npm Trusted Publishing (OIDC)** — `npm publish --access public --provenance` under `permissions: id-token: write`, no stored credential on the path. 🔴 **The trusted publisher must be registered by a human at npmjs.com first, and whether this package's is is UNKNOWN as of 2026-08-14**: package → Settings → Trusted Publisher → GitHub Actions, organization `diagrammo`, repository `dgmo-mcp`, workflow filename `release.yml`, environment blank, allowed action `npm publish`. `scripts/npm-trusted-publishing.sh` walks it. There is no API for it and a bypass-2FA token has been forbidden from changing trusted-publishing config since 2026-07-31, so it cannot be automated at all — until it is registered the publish step fails to **authenticate**. ⚠️ **Do not infer the answer from `npm view @diagrammo/dgmo-mcp dist.attestations`** — an empty attestation proves only that nothing has published from CI, which was true here because releases were run locally. `@diagrammo/dgmo` turned out to be registered all along under exactly that reading. Open the settings page. A non-empty attestation is still the right proof that a release actually went out this way
