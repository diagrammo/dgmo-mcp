...
- It stays `workflow_dispatch` only, with an optional `tag` input; the reason is now "don't run twice", not Actions minutes — this repo is public and Actions on public repos cost nothing (verified 2026-08-14). So a bare tag push still ships nothing. A *Resolve the tag being released* step takes the tag from the input or `GITHUB_REF_NAME` and validates its shape, so a run started the wrong way fails there instead of publishing a version named `main`. (The trigger was first disabled 2026-07-22 over Actions minutes, recorded as "the org npm token is broken" — a **wrong diagnosis**, retracted 2026-07-31 after reading the failing run's own log, which says `You cannot publish over the previously published versions`)
- npm auth is **npm Trusted Publishing (OIDC)** — `npm publish --access public --provenance` under `permissions: id-token: write`, no stored credential on the path. 🔴 **The trusted publisher must be registered by a human at npmjs.com first, and as of 2026-08-14 it is not**: package → Settings → Trusted Publisher → GitHub Actions, organization `diagrammo`, repository `dgmo-mcp`, workflow filename `release.yml`, environment blank, allowed action `npm publish`. `scripts/npm-trusted-publishing.sh` walks it. There is no API for it and a bypass-2FA token has been forbidden from changing trusted-publishing config since 2026-07-31, so it cannot be automated at all — until it is registered the publish step fails to **authenticate**. The proof it has ever published from CI is a non-empty `npm view @diagrammo/dgmo-mcp dist.attestations`
...
- The old local path — publish, then tag — is the fallback while the `~/.npmrc` `local-deploy` token lasts, and it **expires 2026-08-19T02:01Z**. In that flow the publish precedes the tag, so preflight's "already on npm" guard fires; `ALLOW_PUBLISHED=1` downgrades that one check while keeping the rest
