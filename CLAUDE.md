...
- `release.yml`'s tag trigger was disabled 2026-07-22 over the broken org npm token — it is `workflow_dispatch` only, so pushing a tag ships nothing. Use the workspace local-publish path
...
- 🔴 **Never hardcode a chart-type count.** `package.json` says "35+", `server.json` says "40+", `manifest.json` says both — already inconsistent, and off by one within a release. `mcp__dgmo__list_chart_types` is the live source; an open form is the fallback where no command can run
