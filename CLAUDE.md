...
✅ **`check:all` enforces prettier** — `format:check` runs first in the gate, so `pnpm format` is safe to run bare. If `check:all` fails on formatting, run `pnpm format` and commit that **alone** — never as a rider on a feature.
...
🔴 **A dgmo release reaches this package only through an explicit bump.** The range is `>=X <1`, but the committed lockfile pins whatever already satisfied it, so `pnpm install` keeps reporting success while the dep sits releases behind — and because the library is inlined, what resolved is what gets baked in. After any bump, check what actually **resolved** (`grep -A2 "'@diagrammo/dgmo'" pnpm-lock.yaml`), not what was declared.
...
- 🔴 **A tool description says what the tool does and when to call it; it never teaches DGMO syntax** — decided 2026-09-22 on diagrammo/diagrammo#855. That holds for every parameter `.describe()` too. `dgmo/SKILL.md` and the language reference it is generated from are the one authority, and a syntax rule copied in here is prose nothing checks against the spec. Anything about how to write a diagram is a pointer to `get_language_reference`. `tests/tools.test.ts` fails on a description naming a DGMO colour, which is how the one inlined rule this held (the trailing colour token, six copies) got in
